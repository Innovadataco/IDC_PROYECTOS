# PROPUESTA TÉCNICA Y ECONÓMICA

## Estructuración Técnica, Jurídica, Financiera y Tecnológica del Sistema Estratégico de Transporte Público de Sincelejo (SETP)

**Proponente:** D & A Asociados S.A.S.  
**Cliente:** Metro Sabanas S.A.S.  
**Ciudad:** Bogotá D.C., junio 2026

---

# 1. PRESENTACIÓN DEL PROPONENTE

**D & A Asociados S.A.S.** es una empresa especializada en estructuración jurídica, financiera y contractual de proyectos de infraestructura, transporte y contratación pública. Cuenta con oficinas en Bogotá D.C. (Calle 93 No 11a-28, Edificio Capital Park 93, Oficina 603) y asociaciones estratégicas con firmas de abogados en Colombia.

La firma dispone de un equipo multidisciplinario que integra capacidades jurídicas, financieras, técnicas operativas y tecnológicas, lo que permite ofrecer acompañamiento integral en la estructuración de proyectos complejos de movilidad y transporte público.

---

# 2. CONTEXTO DEL PROYECTO

A pesar de los avances alcanzados en la estructuración e implementación del Sistema Estratégico de Transporte Público de Sincelejo (SETP), persiste la necesidad de definir técnicamente los componentes operativos, tecnológicos, jurídicos, financieros y de control requeridos para su entrada en funcionamiento.

El SETP Sincelejo constituye la apuesta estratégica de la ciudad para la transformación de la movilidad urbana, orientada a la prestación de un servicio público de transporte estructurado, regulado, tecnológicamente habilitado y sostenible. El sistema requiere la definición de una arquitectura integral — técnica, contractual, financiera y tecnológica — que permita la operación, control, fiscalización y administración de la flota, rutas, usuarios y recursos del sistema, articulada con los objetivos de planificación territorial y desarrollo sostenible de Sincelejo.

En consecuencia, Metro Sabanas S.A.S., en su calidad de ente gestor del sistema, requiere adelantar una fase de estructuración técnica preliminar orientada a validar alternativas de mercado, definir la arquitectura contractual y tecnológica del proyecto, y consolidar los requerimientos que servirán de base para los futuros procesos de contratación asociados a la provisión de flota, implementación de sistemas tecnológicos, operación del sistema e interventoría.

---

# 3. OBJETO DEL CONTRATO

Prestar servicios profesionales especializados para realizar la **estructuración integral técnica, jurídica, financiera y tecnológica** de los procesos de contratación necesarios para la implementación y puesta en operación del Sistema Estratégico de Transporte Público de Sincelejo (SETP), incluyendo:

- La elaboración de los estudios y documentos requeridos para su preparación y desarrollo;
- La definición, diseño conceptual y especificación de los subsistemas de control, comunicaciones, flota inteligente, plataformas de gestión y sistemas de información ITS;
- El acompañamiento especializado a Metro Sabanas S.A.S. durante las etapas precontractuales y los procesos de selección;
- Garantizando el cumplimiento de estándares técnicos, operativos y de interoperabilidad requeridos para infraestructuras inteligentes de transporte.

---

# 4. ALCANCE DE LOS SERVICIOS

## 4.1 Componente Jurídico-Contractual

- Estructuración jurídica y contractual de los procesos de contratación;
- Análisis de alternativas de estructuración contractual;
- Elaboración de minutas contractuales y documentos de selección;
- Acompañamiento en etapas precontractuales;
- Matrices de riesgos jurídicos.

## 4.2 Componente Financiero

- Análisis financieros requeridos para la estructuración;
- Evaluación de alternativas y modelos de implementación;
- Análisis de sostenibilidad económica del proyecto.

## 4.3 Componente Técnico-Operativo

- Revisión de documentación técnica existente;
- Identificación de actores institucionales, técnicos, financieros y de mercado;
- Realización de mesas técnicas;
- Levantamiento y análisis de información de mercado mediante consulta a potenciales proveedores.

## 4.4 Componente Tecnológico ITS

- Revisión y clasificación de Servicios ITS y subsistemas tecnológicos aplicables al SETP;
- Definición de la arquitectura tecnológica conceptual del SETP (visión de componentes y subsistemas);
- Diseño y especificaciones conceptuales de los componentes tecnológicos y subsistemas ITS;
- Definición de requerimientos funcionales y no funcionales del sistema;
- Concepto de Operación Tecnológica del SETP;
- Análisis del equipamiento de hardware, software y servicios tecnológicos necesarios;
- Apoyo en la elaboración de la Solicitud de Información (RFI) y la Solicitud de Cotización (RFQ) tecnológica.

---

# 5. GOBERNANZA DEL PROYECTO

D & A Asociados S.A.S. designará un **Director del Proyecto** encargado de:

- Gestionar, coordinar y participar en las reuniones requeridas por Metro Sabanas S.A.S.;
- Brindar apoyo técnico y estratégico articulado y coordinado;
- Actuar como interlocutor principal para la validación de entregables;
- Garantizar la integración de los componentes jurídico, financiero, técnico y tecnológico;
- Articular la validación de documentos tecnológicos con actores institucionales, proveedores tecnológicos y grupos de interés.

---

# 6. EQUIPO DE TRABAJO

| Cargo | Perfil | Rol |
|-------|--------|-----|
| **Director Jurídico del Proyecto** | Abogado especialista en derecho del transporte, con experiencia en estructuración jurídica y contractual de proyectos de infraestructura, transporte y contratación pública. | Liderar la estructuración jurídica y contractual del proyecto, coordinar la consultoría, participar en las mesas técnicas, elaborar y revisar los documentos jurídicos requeridos, formular recomendaciones sobre la arquitectura contractual y acompañar los procesos precontractuales y de selección. |
| **Profesional Técnico** | Profesional con experiencia en planeación, estructuración, implementación y/o operación de sistemas de transporte público y sistemas tecnológicos asociados al transporte. | Apoyar el análisis de los componentes técnicos, operativos y tecnológicos del SETP, participar en las mesas técnicas y aportar insumos para la definición de requerimientos y condiciones técnicas de los futuros procesos de contratación. |
| **Profesional Financiero** | Profesional con experiencia en estructuración financiera, evaluación económica y análisis de sostenibilidad de proyectos de transporte e infraestructura. | Apoyar la definición y validación de los componentes financieros requeridos para la estructuración de los procesos de contratación, así como el análisis de alternativas y modelos de implementación desde la perspectiva económica y financiera. |
| **Profesional Tecnológico ITS** | Profesional especializado en sistemas ITS, arquitectura tecnológica, ingeniería de sistemas y diseño de soluciones tecnológicas para transporte inteligente. | Liderar la definición, diseño conceptual y especificación de los componentes tecnológicos ITS del SETP; revisar y clasificar subsistemas ITS; definir arquitectura tecnológica conceptual; especificar requerimientos funcionales y no funcionales; elaborar RFI/RFQ tecnológica. |
| **Apoyo Administrativo** | Técnico, tecnólogo o profesional con experiencia en gestión administrativa y documental. | Apoyar la programación y logística de reuniones, la organización y consolidación de información, el control documental, la elaboración de actas, el seguimiento de actividades y demás labores operativas. |

---

# 7. PRODUCTOS, ENTREGABLES Y FORMA DE PAGO

El valor del contrato será cancelado contra la entrega y aprobación de los siguientes productos:

---

## Producto No. 1. Plan de Trabajo y Estrategia de Estructuración

**Porcentaje de pago: 40%**

Dentro de los primeros cinco (5) días de ejecución, se entregará un documento que contenga:

| ID | Actividad | Semana |
|----|-----------|--------|
| PT-01 | Revisión documental inicial | Semana 1 (5 días hábiles) |
| PT-02 | Identificación de actores y stakeholders | Semana 1 |
| PT-03 | Mesa técnica inicial con Metro Sabana | Semana 1 |
| PT-04 | Definición de metodología de trabajo | Semana 1 |
| PT-05 | Elaboración del Plan de Trabajo | Semana 1 |
| PT-06 | Validación del Plan de Trabajo con Metro Sabana | Semana 1 |

**Incluye:** Plan de trabajo y cronograma de ejecución; metodología para el desarrollo de la consultoría; identificación de actores institucionales, técnicos, financieros y de mercado; estrategia para el desarrollo de mesas técnicas y consultas al mercado; identificación preliminar de aspectos regulatorios, técnicos, financieros y contractuales; ruta metodológica para la estructuración de los procesos de contratación.

**Pago:** Contra entrega y aprobación del Producto 1.

---

## Producto No. 2. Documento de Análisis y Estructuración Preliminar

**Porcentaje de pago: 30%**

| ID | Actividad | Semana |
|----|-----------|--------|
| **Servicios ITS y Subsistemas** | | |
| ITS-01 | Recopilación de normativa ITS aplicable | Semana 2-3 |
| ITS-02 | Revisión de documentación técnica existente | Semana 1-2 |
| ITS-03 | Identificación de subsistemas ITS aplicables | Semana 2 |
| ITS-06 | Documento de revisión y clasificación | Semana 2 |
| **Arquitectura Tecnológica Conceptual** | | |
| ARC-01 | Definición de principios arquitectónicos | Semana 2 |
| ARC-02 | Diseño de vista de componentes | Semana 2-3 |
| ARC-03 | Documento de arquitectura conceptual | Semana 3 |
| **Concepto de Operación** | | |
| COP-01 | Definición de roles y responsabilidades operativos | Semana 2-3 |
| COP-02 | Definición de procesos operativos | Semana 3 |
| COP-03 | Definición de indicadores de desempeño (KPIs) | Semana 3 |
| COP-04 | Definición de escenarios de operación | Semana 3 |
| COP-05 | Documento de Concepto de Operación | Semana 3 |
| **Especificaciones** | | |
| ESP-01 | Especificación de subsistemas | Semana 3 |
| ESP-06 | Especificación de centro de control | Semana 4 |

**Incluye:** Consolidación de resultados de las mesas técnicas; resultados del análisis de mercado y consultas realizadas; evaluación de alternativas técnicas, financieras y contractuales; arquitectura contractual propuesta; definición preliminar de alcances, obligaciones, responsabilidades e interfaces entre los contratos proyectados; arquitectura tecnológica conceptual del SETP; concepto de operación tecnológica.

**Pago:** Contra entrega y aprobación de los entregables 2.1, 2.2, 3.1 y 3.2.

---

## Producto No. 3. Documentos de Estructuración Precontractual

**Porcentaje de pago: 30%**

| ID | Actividad | Semana |
|----|-----------|--------|
| **Requerimientos** | | |
| REQ-01 | Definición de requerimientos funcionales | Semana 3-4 |
| REQ-02 | Definición de requerimientos no funcionales | Semana 3 |
| REQ-03 | Validación de requerimientos con Metro Sabana | Semana 3-4 |
| **RFI** | | |
| RFI-01 | Análisis de mercado tecnológico | Semana 3-4 |
| RFI-02 | Definición de alcance del RFI | Semana 4 |
| RFI-03 | Elaboración del RFI | Semana 4 |
| **RFQ** | | |
| RFQ-01 | Definición de alcance del RFQ | Semana 4 |
| RFQ-02 | Elaboración del RFQ | Semana 4 |
| RFQ-03 | Validación de RFI/RFQ con Metro Sabana | Semana 4 |

**Incluye:** Estudios previos; estudios del sector; matrices de riesgos; análisis financieros requeridos; documentos soporte de la estrategia contractual; anexos técnicos; documentos de selección y minutas contractuales; requerimientos funcionales y no funcionales del sistema; RFI y RFQ tecnológica.

**Pago:** Contra entrega y aprobación de los productos 3.3 y 3.4.

---

# 8. CRONOGRAMA DE EJECUCIÓN

| Fase | Actividad | Semana |
|------|-----------|--------|
| Fase 1: Inicio y Diagnóstico | Revisión documental, mesas técnicas iniciales, identificación de actores y requerimientos | **1** |
| Fase 2: Diseño Conceptual | Definición de arquitectura, subsistemas, especificaciones funcionales y no funcionales | **2-3** |
| Fase 3: Análisis Tecnológico | Apoyo elaboración y análisis del RFI y RFQ | **3-4** |

**Plazo total de ejecución:** Noventa (90) días calendario, contados a partir del cumplimiento de los requisitos de perfeccionamiento y legalización del contrato.

---

# 9. CONDICIONES DE EJECUCIÓN Y APROBACIÓN DE ENTREGABLES

## 9.1 Lugar de Ejecución

Todos los productos, documentos técnicos, análisis, estudios y demás entregables serán desarrollados desde la ciudad de Bogotá D.C., sin perjuicio de la realización de reuniones virtuales o presenciales que sean requeridas por Metro Sabanas S.A.S. En caso de requerirse desplazamientos presenciales fuera de Bogotá D.C., los costos de transporte, alojamiento y viáticos serán asumidos por Metro Sabanas S.A.S. o acordados previamente entre las partes.

## 9.2 Revisión y Aprobación de Entregables

Una vez entregado cada producto, Metro Sabanas S.A.S. contará con un plazo máximo de cinco (5) días hábiles para realizar la revisión correspondiente y comunicar por escrito las observaciones, ajustes o correcciones. En caso de que, transcurrido dicho término, no se reciban observaciones, el entregable se entenderá aprobado a satisfacción para todos los efectos contractuales y de facturación.

## 9.3 Ajustes a los Entregables

Las observaciones presentadas por Metro Sabanas S.A.S. deberán corresponder al alcance definido en la presente propuesta. Los ajustes que resulten procedentes serán atendidos y la versión ajustada entregada para su validación final.

---

# 10. OBLIGACIONES DE SUMINISTRO DE INFORMACIÓN

Metro Sabanas S.A.S. suministrará oportunamente la información técnica, operativa y documental requerida para el desarrollo de las actividades. Los retrasos derivados de la entrega tardía de información podrán generar ajustes en el cronograma inicialmente establecido.

---

# 11. PROPIEDAD INTELECTUAL

Los documentos, análisis, diseños conceptuales y demás productos elaborados en ejecución de la presente propuesta serán entregados a Metro Sabanas S.A.S. para su utilización dentro del proyecto SETP Sincelejo. D & A Asociados S.A.S. conservará la titularidad sobre sus metodologías, modelos, herramientas, plantillas y conocimiento especializado utilizados para la elaboración de los productos.

---

# 12. VALOR Y FORMA DE PAGO

| Concepto | Valor TOTAL (COP) |
|----------|-------------------|
| Estructuración integral técnica, jurídica, financiera y tecnológica del SETP Sincelejo — Incluyendo componente ITS, arquitectura tecnológica, especificaciones, RFI/RFQ y acompañamiento precontractual | **A definir** |
| **Valor total (IVA + impuestos incluidos)** | **A definir** |

**Validez de la oferta:** 30 días calendario.

**Forma de pago:** Contra factura, desglosado por productos:

| Producto | Porcentaje | Valor (COP) | Condición |
|----------|-----------|-------------|-----------|
| Producto 1: Plan de Trabajo y Estrategia | 40% | A definir | Contra entrega y aprobación |
| Producto 2: Análisis y Estructuración Preliminar | 30% | A definir | Contra entrega y aprobación |
| Producto 3: Documentos de Estructuración Precontractual | 30% | A definir | Contra entrega y aprobación |
| **TOTAL** | **100%** | **A definir** | |

---

# 13. EXCLUSIONES DEL ALCANCE

No hacen parte de la presente consultoría:

- La ejecución de actividades propias de la etapa contractual o poscontractual;
- La interventoría, supervisión o administración contractual;
- La defensa judicial o extrajudicial de la entidad;
- La representación en actuaciones administrativas o jurisdiccionales;
- La implementación física de sistemas tecnológicos;
- La operación del sistema post-implementación.

---

# 14. DECLARACIONES

El proponente manifiesta expresamente no estar incurso en ninguna de las causales de inhabilidad e incompatibilidad establecidas en la Ley 1881 de 2016, incluida la derivada de la Ley 1918 de 2018.

---

# 15. REPRESENTANTE LEGAL

---

**D & A Asociados S.A.S.**  
**Representante Legal:** Carolina Durán Negrete  
**Dirección:** Bogotá, D.C. Calle 93 No 11a-28 oficina 603, Edificio Capital Park 93  
**Teléfonos:** +57 (1) 7560757 — Móvil: +57 3004625724  
**Email:** info@duranyalvarez.com

---

> **"La estructuración que no integra tecnología al transporte del siglo XXI, no es estructuración: es nostalgia con anexos."**
